...
The correction methods discussed in Chapter \ref{StandardCalibrations} allow us to use two spatially independent gain factors to convert the S1 and S2 signals into the number of photons and the number of electrons produced by an event anywhere in the detector. In this chapter, we will discuss the physics that leads to the S1 and S2 signals before presenting a combined energy model which uses these two observables to determine the energy deposited during an event. 


\subsection{Recombination Physics} \label{RecombSec} 

%MERGE THIS SECTION WITH THE ONE BELOW IT

Energy from a particle interaction produces excited xenon atoms, ionized xenon atoms, and heat.  The heat released in the interaction is not observed in LUX. Heat is released by soft elastic recoils with xenon atoms. For electron recoil events this lost energy is negligible, but for nuclear recoil it is not.
...
excited xenon ( Xe$^*$) atom combines with a ground state xenon atom to form a xenon dimer Xe$^*_2$

Dimer relaxes to two ground state Xe atoms, producing a 178 nm VUV photon as scintillation light (S1)

Free electrons have a chance to recombine with the positively charge xenon ions, producing additional excitons and more S1 light. 

With the probability of an ion recombining represented as $R$, the number of photons produced in an interaction is given by the initial number of excitons produced plus the number of ions which recombine,
\begin{equation}
N_\gamma = N_{ex} + R \times N_i = N_ion \times (\alpha + R).
\end{equation}

The probability of recombination depends on the electric field, a property which is significant for chapter ??


%S2 signal

Free electrons which do not recombine drift to the liquid surface and produce S2

The number of electrons produced in an interaction is given by the fraction of ions which do not recombine,
\begin{equation}
N_e = N_i(1-R)
\end{equation}

Nuclear recoils are more densely ionizing than electron recoil, leading to more recombination, and therefore more S1 and less S2 signal.  This allows for discrimination discussed in section ??

%Make sure this section covers S2=g2*n_e=g2*(1-R)*N_ion, S1=g1*n_photons=g1*(N_ex + R*N_ion)=g1*(\alpha + R)*N_ion... show this after g1 and g2 are discussed, and reference the above equations


%This gets into the energy model... need to merge it with the section below

The total energy deposited is given by
\begin{equation}
E = \mathcal{L}^{-1} W (N_{ex} + N_i)
\end{equation}
where $N_{ex}$ is the number of excitons created, $N_i$ is the number of ions created, $\mathcal{L}$ is the Lindhard factor which compensates for heat loss, $W$ and is work function for the creation of excitons and ions.

%Talk about linhard factor here
For ER events, L equals one, for NR events L ranges between 0.1 and 0.2.
...
%Other relevant equations that will be useful for Krypcal...

In terms of the exciton to ion ratio given by 
\begin{equation}
\alpha \equiv \frac{ N_{ex}}{N_i}
\end{equation}
we can write the number of ions and number of excitons produced in an interaction as
\begin{equation}
N_i= \frac{E \mathcal{L}}{W (\alpha + 1)}
\end{equation}
\begin{equation}
N_{ex}=\alpha N_i.
\end{equation}
...
%Random shit... maybe useful when talking about SE size later on with g2
number of photons produced is given by
\begin{equation}
\frac{n_{ph}}{x} = (0.140 \frac{E_e}{N} \times 10^{17} -0.474) \times N \times 10^{-17}
\end{equation}
where $n_{ph}$ is the number of photons produced per ionization electron, x is the mean free path of the electron to produce one photon in the xenon gas, $E_e$ is the electric field in V/cm, and $N$ is the number density of the gas in atoms/cm$^3$.
...
\bibitem{TritiumSpec} J.  J.  Simpson.   Measurement  of  the energy  spectrum  of
$^3$H  to determine the antineutrino mass. Phys. Rev. D, 23:649-662, Feb 1981. doi:10.1103/PhysRevD.23.649.

\bibitem{TritiumSpec2} P Venkataramaiah, K Gopala, A Basavaraju, S S Suryanarayana,
and H Sanjeeviah. A simple relation for the fermi function. Journal of Physics G: Nuclear Physics, 11(3):359, 1985.

\bibitem{TritiumSpec3} A Dobi.  Tritium Beta Shape. LUX DocDB. (2015) http://teacher.pas.rochester.edu:8080/wiki/pub/Lux/LuxDB00000402/TritiumBetaShape.pdf

\bibitem{Phelps} P Phelps.  The LUX Dark Matter Experiment: Detector Performance and Energy Calibration.  Ph.D. thesis.  Case Western Reserve University.  (August 2014)
