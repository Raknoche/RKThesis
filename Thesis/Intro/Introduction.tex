...
%WHEN YOU SHOW PLOTS HERE - GET THE UPDATED PLOTS FROM JON. GIVE JON CREDIT IN THIS SECTION SOMEHOW... MAYBE HE HAS A LUX NOTE TO REFERENCE?
...
we loosely constrain $0.0001 \frac{cm}{\sqrt{day}} \le G \le 0.01  \frac{cm}{\sqrt{day}}$. (Figure ??) The value of G was further constrained to $G \le 0.0002 \frac{cm}{\sqrt{day}}$ by injecting natural methane into LUX and observing the effects of outgassing with the sampling system. 

%Include fitting G to UMD data plot here (fig 13 in first draft of tritium paper)
...
choose $D = 2.3 \times 10^{-9} \frac{cm^2}{sec}$ such that the
half-infinite boundary conditions in our diffusion model is valid, and combine this with our allowed range of values for $G$ to extract a value for $K$. We find that an initial injection activity of ?? Bq results in the background rate returning to $<$ 5\% of its initial value in less than one ??. (Figure ??)

%Insert output of Jon's simulation code here, and fill in the ?? as well.

\subsection{ER Band Calibration of the LUX Detector} 

%Describe the LUX injection system here?

The LUX collaboration took many precautions to ensure a safe and successful tritium injection. Prior to injecting any tritiated methane into the detector, a natural methane injection was preformed to measure the purification time constant in LUX and constrain the value of $G$ further.   Twenty milligrams of natural methane were injected into LUX using the CH$_3$T injection system. The sampling system was used to measure the concentration of methane in the detector over the next few days, and a purification time constant of 5.90 $\pm$ 0.07 hours was measured.  The first natural methane injection was not large enough to observe outgassing from the detector, but a larger natural methane injection of ?? grams was able to constrain $G \le 0.0002 \frac{cm}{\sqrt{day}}$ at a later date.


%Show Fig 15 CH4 injection from July 23 2013

\begin{figure} [!h]
\includegraphics[scale=.7]{LUXOutgassing.png} 
\captionof{figure}{Sampling system results from the ?? gram natural methane injection which was used to constraint the value of $G$.}
\label{LuxOutgassing}
\end{figure}
...
%Show CH3T fiducial volume rate plots from the paper

After confirming the purification time constant LUX approved the use of CH$_3$T as an internal calibration source.  The initial CH$_3$T calibration was limited to ?? Hz (?? events) due to the uncertainty in the value of the the diffusion constant $G$, but injections as large as ?? Hz (?? events) have been done after the upper limit of $G$ was lowered.  

%Show and discuss mixing plots
%Show and dicuss ER Band calibration
%Dicuss the xenon physics results and cite the paper.
...
\subsubsection{Recombination Physics} \label{RecombSec} 
